...
function buildInboundHeaders({ mailbridgeHostname, sourceIp, spamSource, spamReason, spamScore, finalSpamVerdict, spamSclScore, aiConfirmedSpam }) {
  ...
  let headers = `Received: from ${fromHost}\r\n`
    ...
    + `        ${timestamp}\r\n`;
  headers += `X-Mailbridge-Source-IP: ${sourceIpForHeader}\r\n`;
  headers += `X-Mailbridge-Spam-Source: ${spamSource}\r\n`;
  headers += `X-Mailbridge-Spam-Reason: ${spamReason}\r\n`;
