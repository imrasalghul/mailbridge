...
Mailbridge bridges Cloudflare Email Workers to a local mail server for inbound mail, and can relay trusted SMTP traffic to Cloudflare Email Service or Resend for outbound mail.
...
-> Cloudflare Email Service or Resend
-> recipient
```
...
- Outbound relay support for Cloudflare Email Service or Resend
...
SMTP_RELAY_TLS_CERT_FILE=/app/secrets/smtp-relay.crt
SMTP_RELAY_TLS_KEY_FILE=/app/secrets/smtp-relay.key
```
...
| Variable | Purpose |
|---|---|
| `RELAY_UPSTREAM_PROVIDER` | `cloudflare` or `resend`; defaults to `cloudflare`. |
| `RELAY_API_KEY` | API key for Resend. Leave blank for Cloudflare outbound. |
